...
async fn main() -> Result<()> {
    ...
    let strategy_handle = tokio::spawn(async move {
        ...
        while let Some(features) = feature_rx.recv().await {
            ...
            if let Some(signal) = generated_signal {
                ...
                #[cfg(feature = "dashboard")]
                {
                    let prob_up = if signal.direction == Direction::Up {
                        signal.confidence
                    } else {
                    ...
                    None => {
                        ...
                        let _ = strategy_persistence
                            .save_rejection(RejectionRecord {
                                ...
                                depth_top5: features.orderbook_depth_top5.unwrap_or(0.0),
                                p_market: 0.0,
                                p_model: signal.confidence.clamp(0.01, 0.99),
                                edge_net: 0.0,
                            })
                            .await;
                        #[cfg(feature = "dashboard")]
    ...
    loop {
        ...
        Some(signal) = signal_rx.recv() => {                let signal_id = signal.id.clone();                // ── Paper Trading path ──────────────────────────
            if paper_trading_enabled {                    if let Some(ref engine) = paper_engine {                        // Apply risk checks (still use risk manager for signal quality)
