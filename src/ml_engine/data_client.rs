...
impl PolymarketDataClient {
    ...
    pub async fn fetch_crypto_markets(
        ...
        info!("Fetching {} crypto markets from Polymarket...", 
        ...
        let crypto_keywords = ["bitcoin", "btc", "ethereum", "eth", "solana", "sol", "ripple", "xrp"];
        let crypto_events: Vec<_> = events.into_iter()
            .filter(|e| {
                let text = format!("{} {} {}", 
                    e.title, 
                    e.slug,
    ...
    pub async fn fetch_price_history(
        ...
        if response.status().is_success() {
            ...
            debug!("Fetched {} price points for token {}", data.history.len(), token_id);
            Ok(data.history)
        } else {
    ...
    pub async fn fetch_historical_trades(
        ...
        if response.status().is_success() {
            ...
            warn!("Failed to fetch trades for condition {}: {}", 
                  condition_id, response.status());
            Ok(Vec::new())
        }
    }
    ...
    pub fn parse_market_metadata(&self, question: &str) -> (Asset, Timeframe, String) {
        ...
        let timeframe = if q.contains("15 min") || q.contains("15m") || q.contains("15 minute") {
            Timeframe::Min15
        } else if q.contains("1 hour") || q.contains("1h") || q.contains("hour") && !q.contains("24") {
            Timeframe::Hour1
        } else {
            Timeframe::Min15
        };
        ...
        let direction = if q.contains("higher") || q.contains("above") || q.contains("up") || q.contains("over") {
            "UP"
        } else if q.contains("lower") || q.contains("below") || q.contains("down") || q.contains("under") {
            "DOWN"
        } else {
            "UNKNOWN"
        }.to_string();
    ...
    pub fn calculate_features_from_prices(
        ...
        // RSI simple
        let gains: Vec<f64> = price_values.windows(2)
            .map(|w| if w[1] > w[0] { w[1] - w[0] } else { 0.0 })
            .collect();
        
        let losses: Vec<f64> = price_values.windows(2)
        ...
        // ADX simplificado
        let plus_dm: Vec<f64> = price_values.windows(2)
            .map(|w| if w[1] > w[0] { w[1] - w[0] } else { 0.0 })
            .collect();
        let minus_dm: Vec<f64> = price_values.windows(2)
            .map(|w| if w[1] < w[0] { w[0] - w[1] } else { 0.0 })
            .collect();
        ...
        features.is_15m = if timeframe == Timeframe::Min15 { 1.0 } else { 0.0 };
    ...
    pub async fn convert_events_to_trades(
        ...
        for event in events {
            for market in event.markets {
                ...
                let (asset, timeframe, direction_str) = self.parse_market_metadata(&market.question);
                ...
                for token in &market.tokens {
                    ...
                    let prices = self.fetch_price_history(
                        &token.token_id,
                        start_ts,
                        end_ts,
                        5, // 5 minutos
                    ...
                    let sample = TradeSample {
                        ...
                        asset,
                        timeframe,
                        direction: if direction_str == "UP" { Direction::Up } else { Direction::Down },
                        is_win,
        ...
        info!("Converted {} events into {} trade samples", events_len, samples.len());
        Ok(samples)
    }
    ...
    pub async fn download_historical_dataset(
        ...
        info!("🚀 Downloading historical dataset (target: {} samples)...", min_samples);
        ...
        for batch in 0..max_batches {
            ...
            info!("Fetching batch {} (have {} samples so far)...", batch + 1, all_samples.len());
        ...
        info!("✅ Download complete: {} historical trade samples", all_samples.len());
